...
def main(cfg: DictConfig):
    ...
    # PATHS
    path_last = f'{cfg.path_inpainting_done}last/'
    path_orig = f'{cfg.path_inpainting_done}orig/'
    path_masks = f'{cfg.path_inpainting_done}masks/'
    path_mask_nodules = f'{cfg.path_inpainting_done}masks_nodules/'
    path_boxes_coords = f'{cfg.path_inpainting_done[:-7]}box_coords/'
    ...
    for idf, f in tqdm(enumerate(files), total=len(files)):
        ...
        lungs, ndls_centers, mask = load_resampled_image(f, cfg.path_data)
        ...
        for idx_cube, (cube_orig, cube_inpain, mask_ndl, z,y,x) in enumerate(zip(cubes_gan_orig, cubes_gan_inpain, mask_ndls, zzs, yys, xxs)):
            ...
            # save files
            df_lidc_ndl.to_csv(f'{cfg.path_dest}lidc_info/{f_name[:-3]}csv', index=False)
            cube_orig.tofile(f'{cfg.path_dest}original/{f_name}')
            cube_inpain.tofile(f'{cfg.path_dest}inpainted_inserted/{f_name}')
            mask_ndl.tofile(f'{cfg.path_dest}mask/{f_name}')
